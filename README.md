...
the append-only delta past the last forwarded prefix - with two principled
exceptions: stale-Read stubbing may reach into the prefix when the cache is
already missed (prefix changed, cold session, or the ~5-minute cache TTL has
lapsed - a dead entry protects nothing), and read maturation replaces bytes
that were deliberately *held out* of the cache in the first place (below).
Blocks carrying `cache_control` are never touched at all, except by the
maturation breakpoint relocation, which moves them by design.
...
cap). After expiry the model can always just re-run the tool. Keys are
session-scoped (the session id is folded into the hash), so identical content
in two sessions never shares a record - a cross-project provenance lesson
borrowed from headroom.

## Adaptive text crushing (Kneedle)

Line-based tool output (Bash, Grep - never file-content tools, and never
results the proxy can't attribute to a tool) gets the same treatment as JSON
arrays, but the keep-count is statistical instead of fixed: `lib/adaptive.mjs`
builds the cumulative unique-bigram coverage curve over the lines and keeps up
to the Kneedle knee - the point where more lines stop adding information. A
SimHash near-duplicate check collapses pure log spam fast, and genuinely
diverse content comes back with K ≈ n and is left alone, so dense unique
output is never truncated. Error-keyword lines and a small tail anchor always
survive; the original is CCR-stored and the marker carries the curl command.

## Read maturation (hold back, then compress)

The biggest context cost in a coding session is old file reads. Mutating an
already-cached Read would bust the prefix cache - but bytes that never entered
the cache have no entry to bust. So a fresh large Read (≥ 2KB) is *held out*
of the provider cache: the trailing `cache_control` breakpoint is parked just
before it while the file is being actively read/edited, and the model sees the
verbatim content the whole time. Once the file has been quiet for 5 assistant
turns (20-turn cap for perpetually busy files), the read matures into a small
CCR-backed marker - and only that final form is ever cache-written:

```
[Read of /path/file.ts compressed after quiesce — re-read the file if needed. Full original: curl -s http://localhost:8787/ccr/<key>]
```

Matured markers replay byte-identically from session state on every later
request, so the cached prefix stays stable for the rest of the session. The
recovery path is the model's existing habit: the file is still on disk, and
the full original is one curl away.

## Cache-miss attribution

Every response's cache fields are classified against what the previous turn
left cached: an expected hit that missed is attributed to `ttl_expiry` (idle
past the ~5-minute cache lifetime - TTL wins ties), `prefix_change` (the
forwarded bytes stopped matching), or `unknown` (provider-side). Counts appear
in `/stats` (`cache_misses`) and as `agentproxy_cache_miss_total{reason=...}`
in `/metrics`, so you can tell "we broke the prefix" from "the session just
went idle" before blaming the proxy.
