...
export function createLedger({ dir, prices }) {
  ...
  const routing = { tiers: { SIMPLE: 0, MODERATE: 0, COMPLEX: 0, REASONING: 0 },
    applied: 0, potential_saved_usd: 0 };
  // Expected-hit misses, bucketed by attributed reason (see proxy.mjs).
  const cacheMisses = { ttl_expiry: 0, prefix_change: 0, unknown: 0 };
  ...
  const all = { id: "all", started: null, requests: 0, saved_tokens: 0,
    saved_detail: { dedup: 0, stale_read: 0, crush: 0, crush_text: 0, mature: 0 },
  ...
  function add(e) {
    ...
    if (ok && e.saved_detail) {
      ...
      s.saved_detail.crush += e.saved_detail.crush ?? 0; // pre-crush lines lack the key
      s.saved_detail.crush_text += e.saved_detail.crush_text ?? 0;
      s.saved_detail.mature += e.saved_detail.mature ?? 0;
    }
    s.cost_with += withUsd;
    ...
    if (ok && e.saved_detail) {
      ...
      all.saved_detail.crush += e.saved_detail.crush ?? 0;
      all.saved_detail.crush_text += e.saved_detail.crush_text ?? 0;
      all.saved_detail.mature += e.saved_detail.mature ?? 0;
    }
    ...
    if (e.cache_miss?.is_miss && cacheMisses[e.cache_miss.reason] != null) {
      cacheMisses[e.cache_miss.reason] += 1;
    }

    // Routing decision (optional field — pre-routing lines aggregate fine).
    // Potential is "≈": priced at list rates, ignores the cache mix.
    if (e.route) {
      if (routing.tiers[e.route.tier] != null) routing.tiers[e.route.tier] += 1;
      if (e.route.applied) routing.applied += 1;
      if (e.route.target && e.route.target !== e.route.from) {
        const fp = resolvePrice(e.route.from, prices);
        const tp = resolvePrice(e.route.target, prices);
        if (fp && tp) {
          const inTok = (e.usage?.input ?? 0) + (e.usage?.cache_read ?? 0) + (e.usage?.cache_creation ?? 0);
          routing.potential_saved_usd +=
            inTok * ((fp.input_cost_per_token ?? 0) - (tp.input_cost_per_token ?? 0)) +
            (e.usage?.output ?? 0) * ((fp.output_cost_per_token ?? 0) - (tp.output_cost_per_token ?? 0));
        }
      }
    }
